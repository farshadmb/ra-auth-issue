import { AuthProvider } from "react-admin"

// eslint-disable-next-line @typescript-eslint/no-unused-vars
const loginUrl = "/login"

export type AuthenticationProvider = AuthProvider & {
  forgotPassword: (params: {
    email?: string
    username?: string
  }) => Promise<void>
  resetPassword: (params: {
    token?: string
    password?: string
  }) => Promise<void>
}

/**
 * Called when app need to login into accountb by `AuthenticationProvider`
 * @param params contains { username, password, redirectTo }
 * @return Promise<void>
 */
const login = (params: any) => {
  const { username, password, redirectTo } = params
  console.log("loging called", params)
  console.log(username, password, redirectTo)
  // TODO: implemented method for login method
  localStorage.setItem("authroized", "true")
  return Promise.resolve()
}
/**
 * Called when app need to logout by `AuthenticationProvider`
 * @param params contains { }
 * @return Promise<void> on success otherwise rejected with error
 */
const logout = (params: any) => {
  // clear the session data if needed
  console.log("logout called", params)
  localStorage.removeItem("authroized")
  // TODO: implemented method for logout method
  return Promise.resolve()
}

/**
 * Called when the use request to change password by `AuthenticationProvider`
 * @param params contains { }
 * @return Promise<void> on success otherwise rejected with error
 */
const resetPassword = (params: any) => {
  console.log("resetPassword called", params)
  // TODO: implemented method for reset password method
  return Promise.resolve()
}

/**
 * Called when the use request to reset password by `AuthenticationProvider`
 * @param params contains { }
 * @return Promise<void> on success otherwise rejected with error
 */
const forgotPassword = (params: any) => {
  console.log("forgotPassword called", params)
  // TODO: implemented method for forgot password method
  return Promise.resolve()
}

const permissions = (params: any): Promise<any> => {
  console.log("permissions called", params, params.admin)
  // TODO: implemented method permissions
  return Promise.resolve({
    user: { resetPassword: true, forgotPassword: true },
  })
}

const userIdentity = () => {
  console.log("userIdentity called")
  // TODO: implemented method user identity
  const authrozied = localStorage.getItem("authrozied")
  if (!authrozied) return Promise.reject()
  if (authrozied !== "true") {
    return Promise.reject()
  }
  const user = {
    id: 1,
    fullname: "Farshad M",
    avatar:
      "data:image/jpeg;base64,",
  }
  return Promise.resolve(user)
}

const onAuthError = (params: any) => {
  console.log("onAuthError called", params)
  return Promise.reject("authError: no authenticaed user")
}

// const basename = process.env.PUBLIC_URL

const checkAuthentication = (params: any) => {
  console.log("checkAuthentication called", params)

  if (params && params.forgetPassword === true) {
    return Promise.resolve()
  }

  const authrozied = localStorage.getItem("authrozied")
  if (authrozied && authrozied === "true") {
    return Promise.resolve()
  }

  return Promise.reject({
    message: "session expired",
  })
}

export const appAuthenticationProvider: AuthenticationProvider = {
  login: login,
  logout: logout,
  getPermissions: permissions,
  getIdentity: userIdentity,
  resetPassword: resetPassword,
  forgotPassword: forgotPassword,
  checkError: onAuthError,
  checkAuth: checkAuthentication,
}

// export const authProvider: AuthProvider = {
//   login: login,
//   logout: logout,
//   getPermissions: permissions,
//   getIdentity: userIdentity,
//   resetPassword: resetPassword,
//   forgotPassword: forgotPassword,
//   checkError: onAuthError,
//   checkAuth: checkAuthentication,
// }

export default appAuthenticationProvider
